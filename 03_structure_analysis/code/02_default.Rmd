...
##### 2c.5. Convert STRUCTURE files into aligned Q files compatible with analysis in the program CLUMPP (Jakobbson and Rosenberg 2007); run the rmd chunk below
...
##### 2c.6. Export CLUMPP compatible files for CLUMPP analysis; run the rmd chunk below to export CLUMPP associated files for each K.
...
clumppExport(align, 
            parammode = 3, 
            paramrep = 10000,
            exportpath = "../data/structure_data/clumpp_data/default_clumpp")
```
...
##### 2c.3. Generate merged Q files using CLUMPP.
...
###### 2c.3.1. Downolad CLUMPP and install on cluster; navigate to the desired directory: `code`. Run the command: `wget https://rosenberglab.stanford.edu/software/CLUMPP_Linux64.1.1.2.tar.gz`

###### 2c.3.2. Run the command: `gunzip CLUMPP_Linux64.1.1.2.tar.gz`

###### 2c.3.3. Run the command: `tar -xvf CLUMPP_Linux64.1.1.2.tar`
cd 
...
###### 2c.3.5. For each analysis separately, execute CLUMPP on the paramfile; run the code `./CLUMPP paramfile`
...
default_k2 <- readQ("../data/structure_data/clumpp_data/default_clumpp/pop_K2/pop_K2-combined-merged.txt")
...
Here, we split the full dataset according to the two major clusters inferred from genetic structure analysis of the full dataset (Analysis 1 above). Analysis 1 split individuals into two major groups, broadly corresponding to subsepecies designation: Neosho Smallmouth Bass, and Northern Smallmouth Bass (plus Spotted Bass). Spotted Bass clustered with Northern Smallmouth Bass in this analysis and so are included within this sample set.

For The Neosho Smallmouth Bass subset, we are running structure with a maximum (<i>K</i>=10), and within the Northern Smallmouth Bass subset, we are running structure with a maximum (<i>K</i>=5).
...
##### 2c.5. Convert STRUCTURE files into aligned Q files compatible with analysis in the program CLUMPP (Jakobbson and Rosenberg 2007)

###### 2c.5.1. Convert STRUCTURE files for Neosho Smallmouth Bass samples only; run the Rmd chunk below.
...
###### 2c.5.2. Convert STRUCTURE files for Northern Smallmouth Bass samples only; run the Rmd chunk below.
