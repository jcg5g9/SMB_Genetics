...
species <- ggplot(pca_df_all, aes(x = PC1, y = PC2, color = Taxon)) + 
  geom_point(size = 2.2, show.legend = T) + 
  scale_color_manual(values = c("springgreen3", "mediumpurple", "firebrick3")) + 
  labs(color = "Species", x = "PC1 (3.3 %)", y = "PC2 (2.8 %)") + 
...
sub <- ggplot(pca_df_nospb, aes(x = PC1, y = PC2, color = Taxon)) + 
  geom_point(size = 2.2, show.legend = T) + 
  scale_color_manual(values = c("springgreen3", "mediumpurple")) + 
  labs(color = "Subspecies", x = "PC1 (3.5 %)", y = "PC2 (1.8 %)") + 
  theme_set(theme_cowplot(12)) + 
...
drain <- ggplot(pca_df_nospb, aes(x = PC1, y = PC2, color = Wang_Structure_Parameters)) + 
  geom_point(size = 2.2, show.legend = T) + 
  scale_color_manual(values = c("springgreen3", "forestgreen", "chocolate1","mediumblue")) + 
  labs(color = "Drainage", x = "PC1 (3.5 %)", y = "PC2 (1.8 %)") + 
  theme_set(theme_cowplot(12)) + 
...
pop <- ggplot(pca_df_nospb, aes(x = PC1, y = PC2, color = Pop)) + 
  geom_point(size = 2.2, show.legend = F) + 
  scale_color_manual(values = c("forestgreen", "lightskyblue", "darkorchid1", "blue", "chocolate1", "gold1", "maroon1", "lightpink1")) +
...
pdf("/Users/joegunn/Desktop/Grad_School_Stuff/Research/Projects/Smallmouth_Bass_Microsatellites/visualization/genetic_pca_figures/genpcas_manuscript.pdf", width=15, height=12)

plot_grid(species, sub, drain, pop, nrow = 2, ncol = 2, labels = c("A", "B", "C", "D"), label_size = 25)
